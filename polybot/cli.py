...
def _print_mm_report(report, *, title: str, elapsed_secs: float | None = None) -> None:
    ...
    if report.top_tokens:
        typer.echo("  Best tokens by total (spread + rewards):")
        for t in report.top_tokens:
            typer.echo(f"    {t.token_id[:14]}… total ${t.total():+.2f} "
                       f"(spread ${t.equity():+.2f}, rw ${t.rewards:.2f}) fills {t.fills}")
    if report.worst_tokens:
        typer.echo("  Worst tokens by total (where the bleed is):")
        for t in report.worst_tokens:
            typer.echo(f"    {t.token_id[:14]}… total ${t.total():+.2f} "
                       f"(spread ${t.equity():+.2f}, rw ${t.rewards:.2f}) inv {t.inventory:+.0f}")
    typer.echo("─" * 60)
